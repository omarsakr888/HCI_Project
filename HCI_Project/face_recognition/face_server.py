...
import time
import traceback
from deepface import DeepFace  # ✅ added
...
def handle_register(conn):
    ...
    try:
        ...
        # ✅ updated to support new/old structure
        known_encodings = [
            np.array(db[name]["encoding"]) if isinstance(db[name], dict) else np.array(db[name])
            for name in db
        ]
            ...
            if face_locations:
                ...
                if encodings:
                    ...
                    try:
                        ...
                        print(f"[DeepFace] error (ignored): {e}")
...
def handle_client(conn, addr):
    ...
    try:
        ...
        elif action == "verify" or action == "login":
            handle_verify(conn)
        else:
